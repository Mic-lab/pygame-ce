...
int
PG_ConvertSurfaceToPixels(SDL_Surface *src, void *dst, Uint32 dst_format,
                          int dst_pitch)
{
    ...
    if (SDL_ISPIXELFORMAT_INDEXED(src_format) || has_colorkey) {
        int free_src = 0;
        if (src_format != dst_format || has_colorkey) {
            free_src = 1;
        ...
        if (dst_pitch < 0) {
            /* Constructing bottum-up data, dst should be last row of data */
            dst = (void *)((Uint8 *)dst - (src->h - 1) * dst_pitch);
        }

        Uint8 *src_pixels = (const Uint8 *)src->pixels;
        int width = PG_SURF_BytesPerPixel(src) * src->w;
        int height = src->h;
        while (height--) {
            memcpy(dst, src_pixels, width);
            src_pixels = src_pixels + src->pitch;
        ...
        if (do_flip) {
            ...
            Uint8 *row2 = row1 + (src->h - 1) * dst_pitch;
            while (row1 < row2) {
...
image_tobytes(PyObject *self, PyObject *arg, PyObject *kwarg)
{
    ...
    if (doing_premul) {
        ...
        surfobj =
            PyObject_CallMethod((PyObject *)surfobj, "premul_alpha", NULL);
        if (!surfobj) {
            return NULL;
        }
    }
    ...
    if (PG_ConvertSurfaceToPixels(surf, (void *)data, pixfmt, pitch) < 0) {
